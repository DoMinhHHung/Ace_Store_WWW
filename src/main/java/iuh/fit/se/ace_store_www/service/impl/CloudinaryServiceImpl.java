...
public class CloudinaryServiceImpl implements CloudinaryService {
    ...
    public ApiResponse uploadFile(MultipartFile file) {
        if (file.isEmpty()) {
            return ApiResponse.fail("File cannot be empty, man.");
        }
        try {
    ...
    public ApiResponse uploadMultipleFile(MultipartFile[] files) {
        if (files == null || files.length == 0) {
            return ApiResponse.fail("File list cannot be empty, bro.");
        }
        ...
        return ApiResponse.ok("Multiple files uploaded successfully.", uploadedFilesData);
    }

    @Override
    public ApiResponse deleteFile(String publicId) {
        if (publicId == null || publicId.isEmpty()) {
            return ApiResponse.fail("Public ID is required to delete the file, bae.");
        }
        try {
            Map deleteResult = cloudinary.uploader().destroy(publicId, ObjectUtils.emptyMap());
            String result = deleteResult.get("result").toString();
            if (result.equals("ok")) {
                return ApiResponse.ok("File deleted successfully. Public ID: " + publicId, null);
            } else if (result.equals("not found")) {
                return ApiResponse.fail("File not found on Cloudinary with Public ID: " + publicId);
            } else {
                return ApiResponse.fail("Failed to delete file on Cloudinary. Result: " + result);
            }
        } catch (IOException e) {
